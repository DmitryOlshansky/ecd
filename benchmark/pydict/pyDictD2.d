...
*
*
* A comment for whom like me, and are not very familiar with hash table implementations.
* After looking at it for a while, it appears to be
*
...
* that there might be another key with same hash following in the peturb sequence. So the look_dict checks further for a match.
* Therefore an entry with a real hash 1 must be stored independently of the main table.
*/

/* Ported to D2 by David Simcha.*/

class PyDictKeyError : Error {
	this(string msg)
	{
		super(msg);
	}
...
debug {
    ...
    @property public size_t length(){ 	return used + is_dummy + is_unused; }
	...
	*/
	...
	public final V opIndex(K k)
	{
		...
		if (val is null)
		{
			throw new PyDictKeyError("invalid key");
		}
		return *val;
	}
	...
	*/
	...
	private void dictresize(size_t minused)
	{
		...
		//newtable = cast(Entry*) GC.calloc(newsize * Entry.sizeof);
